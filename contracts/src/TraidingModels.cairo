...
use starknet::{
    ...
    use crate::utils::events::{BotAuthorized, FundsDeposited, FundsWithdrawn, ModelUpdated, TradeExecuted, ModelMetricsUpdated};
    ...
    struct Storage {
        ...
        traid_models_metrics: Map<u64, Metrics>,
        traid_models_tokens: Map<u64, (ContractAddress, ContractAddress)>,
    ...
    impl TraidingModelsMetrics of ITraidingModelsMetrics<ContractState> {
        ...
        fn update_model_predictions(ref self: ContractState, model_id: u64, prediction: u128) -> (){
            ...
            let precision = self.traid_models_metrics.entry(model_id).error.precision.read();
            let scaled_prediction = prediction * precision;
            self.traid_models_metrics.entry(model_id).prediction.write(scaled_prediction);
        ...
        fn get_bot_portfolio_value(self: @ContractState, model_id: u64) -> u128 {
            let amount: u128 = self.traid_models_metrics.entry(model_id).portfolio_value.read();
            let (_, token_address): (ContractAddress, ContractAddress) = self.traid_models_tokens.entry(model_id).read();
        ...
        fn get_model_error(self: @ContractState, model_id: u64) -> (u128, u128) {
            let mae = self.traid_models_metrics.entry(model_id).error.mae.read();
            let mse = self.traid_models_metrics.entry(model_id).error.mse.read();
            return (mae, mse);
        }

        fn get_model_roi(self: @ContractState, model_id: u64) -> (u128, bool) {
            let roi = self.traid_models_metrics.entry(model_id).roi.value.read();
            let flag = self.traid_models_metrics.entry(model_id).roi.is_negative.read();
            return (roi, flag);
        }

        fn get_model_max_drawdown(self: @ContractState, model_id: u64) -> u128 {
            let max_drawdown = self.traid_models_metrics.entry(model_id).max_drawdown.max_drawdown.read();
            return max_drawdown;
        }

        fn get_model_winning_ratio(self: @ContractState, model_id: u64) -> u128 {
            let winning_ratio = self.traid_models_metrics.entry(model_id).winning_ratio.ratio.read();
    ...
    impl TraidingModelsOrders of ITraidingModelsOrders<ContractState> {
        ...
        fn withdraw(ref self: ContractState, token_address: ContractAddress,  model_id: u64, amount: u256,
            ...
            self.user_balances.entry(caller).entry(model_id).write(new_balance);
        ...
        fn is_model_authorized(ref self: ContractState, model_id: u64) -> bool {
        ...
        fn is_user_authorized(ref self: ContractState, model_id: u64, user_address: ContractAddress) -> bool {
        ...
        fn execute_trade(ref self: ContractState, model_id: u64, asset_id: ContractAddress, amount: u256) -> bool {
            return false;
        }
    }
    ...
    impl TraidInternalCalculations of ITraidInternalCalculations {
        ...
        fn calculate_model_error(ref self: ContractState, model_id: u64, asset_id: felt252) -> () {
            ...
            let model_metrics = self.traid_models_metrics.entry(model_id);
        ...
        fn calculate_model_roi(ref self: ContractState, model_id: u64, asset_id: felt252) -> () {
            let initial = self.traid_models_metrics.entry(model_id).roi.initial.read();
            let current = self.get_asset_price(asset_id) * initial;
            let mut roi = self.traid_models_metrics.entry(model_id).roi.value.read();
            ...
            if current > initial {
                ...
                self.traid_models_metrics.entry(model_id).roi.is_negative.write(true);
            }
            
            self.traid_models_metrics.entry(model_id).roi.value.write(roi);
        }

        fn calculate_model_max_drawdown(ref self: ContractState, model_id: u64, asset_id: felt252) -> () {
            let metrics = self.traid_models_metrics.entry(model_id);
